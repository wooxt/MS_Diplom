...
mbl <- list()
 mbl$File$Open$handler = function(h,...) { mydata<- chooseFile()
			attach(mydata)
			...
			for (j in 1: length(zY)){
				for (i in 1:nrow(mydata)) {				#переводим факторную переменную в 0 и 1
					if (factor.var[i]== zY[j]) x[i]=1 else x[i]=0}
				y<-x+y}
			...
			if (length(svalue(objY)) != 0){
				...
				mydata<-balans(mydata,y,0,1,
					as.numeric(svalue(N)),as.numeric(svalue(M)))}
				 ...
				 handler = function(...){
					...
					gcombobox(model[1:as.numeric(svalue(nMod))],container = group,
							handler=function(h,...){
									...
									field.name <- attr(attr(terms(formula(logit)), "factors"),
                      									 		"dimnames")[[1]][1]
									eval(parse(text=paste("tmp <- ",
                							 ifelse(class(logit$data) == "data.frame", "logit$data$", ""),
                								 field.name, sep="")))
									...
									res6<-glabel(paste("Чувствительность",":",round(tp/(tp+fp),4),"|||","Специфичность",":",round(tn/(tn+fn),4)),container = g2)				
									res7<-gbutton("Save", container=g2,visible = TRUE, handler = 
														function(h,...){
